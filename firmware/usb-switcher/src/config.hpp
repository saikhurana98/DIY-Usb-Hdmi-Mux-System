 ...
 * 
 */
typedef std::map<String, pair<int, pair<int,int>>> UsbChannelPinoutMap;

/**
 * @brief Map of Channel id and Hdmi Source enum
 * 
 */
typedef std::map<String, UsbSource> UsbChannelSourceMap;
...
public:
    ...
    */
     ...
     */
    UsbChannelPinoutMap *usbChannelPinouts;

    /**
     * @brief State to set the Mux's to on boot. Loaded from flash on boot
     */
    UsbChannelSourceMap restoreState;
     ...
     */
    void setRestoreState(RestoreMode mode, UsbChannelSourceMap map);
    /** 
    ...
    */
     ...
     * @param pinout ChannelPinMap Map of the channel id and the pinout pair (trig, sense)
     * @param restoreMode Default Restore Mode if the config is not present
     */
    Config(int serialRxPin, int serialTxPin, double baud, UsbChannelPinoutMap &pinout, RestoreMode restoreMode);
private:
