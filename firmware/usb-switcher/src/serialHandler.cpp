...
JsonDocument SerialHandler::getSource(JsonDocument &payload)
{
    ...
    returnPayload[channel] = this->usbHandler->getSourceString(channel);
...
JsonDocument SerialHandler::getSourceMulti(JsonDocument &payload)
{
    ...
    for (JsonVariant channel : payload["payload"]["channels"].as<JsonArray>())
    {
        ...
        returnPaylod["payload"][channelString] = this->usbHandler->getSourceString(channel);
    }
...
void SerialHandler::setSource(JsonDocument &payload)
{
    ...
    for (JsonPair channel_source : channels)
    {
        ...
        this->usbHandler->setSource(channel, source);
    }
}
...
void SerialHandler::setRestoreMode(JsonDocument &payload)
{
    ...
    this->usbHandler->setBootRestoreMode(mode,states);
...
SerialHandler::SerialHandler(Config &config, UsbHandler &usbHandler, SerialUART &serial)
{
    this->serial = &serial;
